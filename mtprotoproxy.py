...
# IP 白名单系统
ip_whitelist = set()  # 允许连接的 IP 地址集合
ip_attempt_counter = collections.defaultdict(int)  # 连接尝试次数计数器
IP_WHITE_FILE = "ip_white.yaml"  # 白名单文件路径
IP_REQUIRED_ATTEMPTS = 6  # 加入白名单所需的尝试次数
...
def init_config():
    ...
    if not modes["classic"] and not modes["secure"] and not modes["tls"]:
        print_err("没有启用任何已知模式，启用 TLS 专用模式")
        modes["tls"] = True

    if legacy_warning:
        print_err("检测到旧版配置选项 SECURE_ONLY 或 TLS_ONLY")
        print_err("请在配置文件中使用 MODES 代替:")
        print_err("MODES = {")
...
def apply_upstream_proxy_settings():
    ...
    if config.SOCKS5_HOST and config.SOCKS5_PORT:
        import socks
        print_err("Socket 代理模式已启用，与广告和 uvloop 不兼容")
...
def load_ip_whitelist():
    ...
    global ip_whitelist
    try:
        if os.path.exists(IP_WHITE_FILE):
            import yaml
            with open(IP_WHITE_FILE, 'r', encoding='utf-8') as f:
                data = yaml.safe_load(f)
                if data and 'whitelist' in data and data['whitelist']:
                    ip_whitelist = set(data['whitelist'])
                    print_err("已从白名单加载 %d 个 IP 地址" % len(ip_whitelist))
                else:
                    print_err("白名单文件为空，开始使用空白名单")
        else:
            print_err("白名单文件不存在，创建新文件")
            with open(IP_WHITE_FILE, 'w', encoding='utf-8') as f:
                f.write("# IP \u767d\u540d\u5355\u914d\u7f6e\u6587\u4ef6\n")
                f.write("# \u683c\u5f0f: YAML\n")
                f.write("# \u8bf4\u660e: \u6b64\u6587\u4ef6\u4e2d\u7684 IP \u5730\u5740\u53ef\u4ee5\u76f4\u63a5\u8fde\u63a5\u4ee3\u7406\n")
                f.write("#       \u65b0 IP \u9700\u8981\u5c1d\u8bd5\u8fde\u63a5 6 \u6b21\u540e\u624d\u4f1a\u88ab\u81ea\u52a8\u6dfb\u52a0\u5230\u6b64\u767d\u540d\u5355\n\n")
                f.write("# \u767d\u540d\u5355 IP \u5217\u8868\n")
                f.write("whitelist:\n")
                f.write("  # \u793a\u4f8b: - 192.168.1.100\n")
                f.write("  # \u793a\u4f8b: - 10.0.0.1\n")
    except ImportError:
        print_err("错误: 未安装 PyYAML 模块，请执行: pip install pyyaml")
        print_err("将使用空白名单继续运行")
    except Exception as e:
        print_err("加载 IP 白名单时出错: %s" % e)


def save_ip_to_whitelist(ip):
    """将新 IP 保存到 YAML 白名单文件"""
    try:
        import yaml
        # 读取现有数据
        data = {'whitelist': []}
        if os.path.exists(IP_WHITE_FILE):
            with open(IP_WHITE_FILE, 'r', encoding='utf-8') as f:
                ...
                if existing_data and 'whitelist' in existing_data:
                    data = existing_data
        
        # 添加新 IP
        if data['whitelist'] is None:
            data['whitelist'] = []
        ...
        print_err("✓ IP %s 已添加到白名单" % ip)
    except ImportError:
        print_err("错误: 未安装 PyYAML 模块")
    except Exception as e:
        print_err("保存 IP 到白名单时出错: %s" % e)


def check_ip_whitelist(ip):
    """检查 IP 是否在白名单中，或者在 3 次尝试后应该被添加
    返回: (allowed, message)
    """
    global ip_whitelist, ip_attempt_counter
    
    # 检查 IP 是否已经在白名单中
    if ip in ip_whitelist:
        return True, "IP 已在白名单中"
    
    # 增加尝试计数
    ip_attempt_counter[ip] += 1
    current_attempts = ip_attempt_counter[ip]
    
    print_err("● IP %s 连接尝试 %d/%d" % (ip, current_attempts, IP_REQUIRED_ATTEMPTS))
    
    # 检查是否达到所需尝试次数
    if current_attempts >= IP_REQUIRED_ATTEMPTS:
        # 添加到白名单
        ip_whitelist.add(ip)
        save_ip_to_whitelist(ip)
        # 清理计数器
        del ip_attempt_counter[ip]
        return True, "已完成 %d 次尝试，IP 已加入白名单" % IP_REQUIRED_ATTEMPTS
    else:
        remaining = IP_REQUIRED_ATTEMPTS - current_attempts
        return False, "还需要尝试 %d 次才能加入白名单" % remaining
    ...
    async def read(self, n, ignore_buf=False):
        ...
        while True:
            ...
            if tls_rec_type not in [b"\x14", b"\x17"]:
                print_err("错误: FakeTLSStreamReader 中的错误 TLS 类型 %s" % tls_rec_type)
                return b""

            version = await self.upstream.readexactly(2)
            if version != b"\x03\x03":
                print_err("错误: FakeTLSStreamReader 中的未知版本 %s" % version)
                return b""
...
class MTProtoCompactFrameStreamWriter(LayeredStreamWriterBase):
    ...
    def write(self, data, extra={}):
        ...
        if len(data) % 4 != 0:
            print_err("错误: MTProtoFrameStreamWriter 尝试发送长度为", len(data), "的消息")
            return 0
...
class ProxyReqStreamReader(LayeredStreamReaderBase):
    ...
    async def read(self, msg):
        ...
        print_err("未知的 RPC 响应类型:", ans_type)
        return b"", {"SKIP_SEND": True}
...
class ProxyReqStreamWriter(LayeredStreamWriterBase):
    ...
    def write(self, msg, extra={}):
        ...
        if len(msg) % 4 != 0:
            print_err("错误: 尝试发送长度为 %d 的消息" % len(msg))
            return 0
...
async def do_direct_handshake(proto_tag, dc_idx, dec_key_and_iv=None):
    ...
    try:
        ...
        print_err("尝试连接到 %s %s 时被拒绝" % (dc, TG_DATACENTER_PORT))
        return False
    except ConnectionAbortedError as E:
        print_err("Telegram 服务器连接错误: %d (%s %s) %s" % (dc_idx, addr, port, E))
        return False
    except (OSError, asyncio.TimeoutError) as E:
        print_err("无法连接到 %s %s" % (dc, TG_DATACENTER_PORT))
        return False
...
async def handle_client(reader_clt, writer_clt):
    ...
    # 检查 IP 白名单
    allowed, message = check_ip_whitelist(client_ip)
    if not allowed:
        print_err("⛔ IP %s 未在白名单: %s" % (client_ip, message))
        update_stats(connects_bad=1)
        writer_clt.close()
        return
...
async def stats_printer():
    ...
    while True:
        ...
        for user, stat in user_stats.items():
            print("%s: %d 次连接 (当前 %d 个), %.2f MB, %d 条消息" % (
        ...
        try:
            ...
            for line in headers.split(b"\r\n"):
                ...
                if is_time_skewed and config.USE_MIDDLE_PROXY and not disable_middle_proxy:
                    print_err("检测到时间偏差，请设置系统时钟")
                    print_err("服务器时间:", srv_time, "本地时间:", now_time)
                    print_err("禁用广告以继续服务")
                    print_err("放下防止重放攻击的防御")
...
def print_tg_info():
    ...
    if config.PORT == 3256:
        print("默认端口 3256 正在使用，不推荐这样做", flush=True)
        if not config.MODES["classic"] and not config.MODES["secure"]:
            print("由于您启用了 TLS 专用模式，最佳端口是 443", flush=True)
    ...
    for user, secret in sorted(config.USERS.items(), key=lambda x: x[0]):
        ...
        if secret in ["00000000000000000000000000000000", "0123456789abcdef0123456789abcdef",
            ...
            msg = "正在使用默认密钥 {}，不推荐这样做".format(secret)
            print(msg, flush=True)
            random_secret = "".join(myrandom.choice("0123456789abcdef") for i in range(32))
            print("您可以将其更改为此随机密钥:", random_secret, flush=True)
            print_default_warning = True

    if config.TLS_DOMAIN == "www.google.com":
        print("正在使用默认 TLS_DOMAIN www.google.com，不推荐这样做", flush=True)
        msg = "您应该使用随机的现有域名，错误客户端会被代理到那里"
        print(msg, flush=True)
        print_default_warning = True
...
def setup_files_limit():
    try:
        ...
        print("无法增加打开文件的限制", flush=True, file=sys.stderr)
    except ImportError:
        pass
